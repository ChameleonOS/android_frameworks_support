...
package org.chameleonos.support.view;
...
public class VelocityViewPager extends ViewPager implements GestureDetector.OnGestureListener {
    ...
    private void trackMotion(float distX) {
        ...
        final float rightBound = Math.min(getCurrentItem() + 1, getAdapter().getCount() - 1) * widthWithMargin;
    ...
    private void onUp() {
        if (mFlingRunnable.mScroller.isFinished()) {
            ...
            final float leftBound = Math.max(0, (getCurrentItem() - 1) * widthWithMargin);
            final float rightBound = Math.min(getCurrentItem() + 1, getAdapter().getCount() - 1) * widthWithMargin;
            final float midBound = (leftBound + rightBound) / 2;
